...
- **Policy Guard (`PG001`–`PG004`)** — agents cannot clear `--agent-strict` by widening policy. Compares current `locus.lock` to `git show <baseline>:locus.lock`; fires Fatal under strict on budget raises, untagged overrides, new exempt paths, or new `acknowledged_empty` entries. Calibration is explicit via `--allow-policy-calibration` (renders PG diagnostics as Advisory and prints a structured before/after report). Schema requires `reason` / `expires` / `owner` debt metadata on overrides via PG002. Spec: `docs/superpowers/specs/2026-05-09-policy-guard-paradigm.md` (#44).
