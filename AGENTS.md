...
- ✅ AIR emission v12 (Rust adapter, package-prefixed symbols, clean type rendering, imports, call sites, impl blocks, doc comments, line counts, normalized loader facts, `let _ = ...` silent discards, partial `if let Ok/Err`, `match` arm bodies with shape classification, closure-arg shape on method calls, `// ot: marks <fact_kind>` user-fact markers, `unwrap_or` fallback shapes, retry-shaped loops, scrutinee literals).
