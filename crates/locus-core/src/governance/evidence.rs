//! Typed evidence carried by `RuleFinding`s.
//!
//! Each rule produces structured evidence policies can introspect. New
//! typed variants are added as rules migrate (CX001 → `ComplexityBudget`,
//! OT002 → `InferenceConfidence`, etc.). Migrated rules whose schema
//! isn't typed yet use `Structured`; the legacy adapter uses `Legacy`.
