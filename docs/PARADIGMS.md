...
- general literal capture beyond match-arms / binary-RHS (e.g. literal HashMap initialisers for inline lookup tables)
...
| CF (2) | CF001, **CF002**, **CF003** | many | env-read outside `config_paths`; **magic decision constants in scrutinee position (match-arm patterns or binary-compare RHS) outside config layer**; **hardcoded provider/model/topic IDs matching `forbidden_id_patterns`**. |
...
| BO (5) | BO001, BO002, BO004, **BO005** | many | forbidden imports; persistence types in domain signatures; forbidden derives on canonical types; **persistence-write fact targeting domain function**. |
...
- **Governance diagnostic codes:** `LOCUS001` (expired exception), `LOCUS002` (vacant-by-definition paradigm nudge), `LOCUS003` (migration-debt — legacy rule code lacks a registered `RuleDefinition`), `LOCUS004` (architecture-coherence drift between `.locus/arch.json` and the policy/rule/paradigm registries). `LOCUS003`/`LOCUS004` always fire at Advisory severity — they surface drift for review, never block CI. The MVP framing for `LOCUS004` is deliberate per #75; future iterations can elevate severity once drift behaviour stabilises.
